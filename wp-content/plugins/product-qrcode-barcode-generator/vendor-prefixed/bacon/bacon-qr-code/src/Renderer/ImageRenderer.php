...
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\Image\ImageBackEndInterface;
...
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\RendererStyle\RendererStyle;
final class ImageRenderer implements RendererInterface
{
    public function __construct(private readonly RendererStyle $rendererStyle, private readonly ImageBackEndInterface $imageBackEnd)
    {
    }
    /**
    ...
    private function drawEye(Path $externalPath, Path $internalPath, EyeFill $fill, float $xTranslation, float $yTranslation, int $rotation, Path $modulePath): Path
    {
        if ($fill->inheritsBothColors()) {
            return $modulePath->append($externalPath->rotate($rotation)->translate($xTranslation, $yTranslation))->append($internalPath->rotate($rotation)->translate($xTranslation, $yTranslation));
        }
