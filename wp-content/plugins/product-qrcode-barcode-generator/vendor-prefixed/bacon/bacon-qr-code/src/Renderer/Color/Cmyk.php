...
use ProductQrBarcode\Vendor\BaconQrCode\Exception;
final class Cmyk implements ColorInterface
{
     ...
     */
    public function __construct(private readonly int $cyan, private readonly int $magenta, private readonly int $yellow, private readonly int $black)
    {
        if ($cyan < 0 || $cyan > 100) {
    ...
    public function toRgb(): Rgb
    {
        ...
        $y = $this->yellow / 100;
        $k = $this->black / 100;
        return new Rgb((int) round(255 * (1 - $c) * (1 - $k)), (int) round(255 * (1 - $m) * (1 - $k)), (int) round(255 * (1 - $y) * (1 - $k)));
    }
