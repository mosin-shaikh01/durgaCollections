...
use ProductQrBarcode\Vendor\BaconQrCode\Encoder\ByteMatrix;
use ProductQrBarcode\Vendor\BaconQrCode\Encoder\MatrixUtil;
use ProductQrBarcode\Vendor\BaconQrCode\Encoder\QrCode;
use ProductQrBarcode\Vendor\BaconQrCode\Exception\InvalidArgumentException;
use ProductQrBarcode\Vendor\BaconQrCode\Exception\RuntimeException;
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\Color\Alpha;
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\Color\ColorInterface;
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\RendererStyle\EyeFill;
use ProductQrBarcode\Vendor\BaconQrCode\Renderer\RendererStyle\Fill;
use GdImage;
final class GDLibRenderer implements RendererInterface
{
    ...
    public function __construct(private int $size, private int $margin = 4, private string $imageFormat = 'png', private int $compressionQuality = 9, private ?Fill $fill = null)
    {
    ...
    private function draw(ByteMatrix $matrix): void
    {
        ...
        for ($y = 0; $y < $matrixSize; $y += 1) {
            for ($x = 0; $x < $matrixSize; $x += 1) {
                ...
                $points = $this->normalizePoints([($this->margin + $x) * $pointInPx, ($this->margin + $y) * $pointInPx, ($this->margin + $x + 1) * $pointInPx, ($this->margin + $y) * $pointInPx, ($this->margin + $x + 1) * $pointInPx, ($this->margin + $y + 1) * $pointInPx, ($this->margin + $x) * $pointInPx, ($this->margin + $y + 1) * $pointInPx]);
    ...
    {
        $internalColor = $this->getColor($eyeFill->inheritsInternalColor() ? $this->fill->getForegroundColor() : $eyeFill->getInternalColor());
        $externalColor = $this->getColor($eyeFill->inheritsExternalColor() ? $this->fill->getForegroundColor() : $eyeFill->getExternalColor());
        for ($y = 0; $y < 7; $y += 1) {
            for ($x = 0; $x < 7; $x += 1) {
                if (($y === 1 || $y === 5) && $x > 0 && $x < 6 || ($x === 1 || $x === 5) && $y > 0 && $y < 6) {
                    continue;
                }
                $points = $this->normalizePoints([($this->margin + $x + $xOffset) * $pointInPx, ($this->margin + $y + $yOffset) * $pointInPx, ($this->margin + $x + $xOffset + 1) * $pointInPx, ($this->margin + $y + $yOffset) * $pointInPx, ($this->margin + $x + $xOffset + 1) * $pointInPx, ($this->margin + $y + $yOffset + 1) * $pointInPx, ($this->margin + $x + $xOffset) * $pointInPx, ($this->margin + $y + $yOffset + 1) * $pointInPx]);
                if ($y > 1 && $y < 5 && $x > 1 && $x < 5) {
    ...
    private function getColor(ColorInterface $color): int
    {
        ...
        if (!isset($this->colors[$colorKey])) {
            $colorId = imagecolorallocatealpha($this->image, $rgb->getRed(), $rgb->getGreen(), $rgb->getBlue(), (int) ((100 - $alpha) / 100 * 127));
            if ($colorId === \false) {
