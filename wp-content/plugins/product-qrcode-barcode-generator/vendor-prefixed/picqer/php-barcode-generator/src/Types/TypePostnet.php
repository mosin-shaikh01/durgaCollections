...
class TypePostnet implements TypeInterface
{
    ...
    protected array $barlen = [0 => [2, 2, 1, 1, 1], 1 => [1, 1, 1, 2, 2], 2 => [1, 1, 2, 1, 2], 3 => [1, 1, 2, 2, 1], 4 => [1, 2, 1, 1, 2], 5 => [1, 2, 1, 2, 1], 6 => [1, 2, 2, 1, 1], 7 => [2, 1, 1, 1, 2], 8 => [2, 1, 1, 2, 1], 9 => [2, 1, 2, 1, 1]];
    public function getBarcode(string $code): Barcode
    {
        ...
        $len = strlen($code);
        // start bar
        $barcode->addBar(new BarcodeBar(1, 2, \true));
        $barcode->addBar(new BarcodeBar(1, 2, \false));
        for ($i = 0; $i < $len; ++$i) {
            for ($j = 0; $j < 5; ++$j) {
                $h = $this->barlen[$code[$i]][$j];
                $p = (int) floor(1 / $h);
                $barcode->addBar(new BarcodeBar(1, (int) $h, \true, $p));
                $barcode->addBar(new BarcodeBar(1, 2, \false));
            }
        }
        // end bar
