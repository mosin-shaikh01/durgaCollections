...
use ProductQrBarcode\Vendor\Picqer\Barcode\Exceptions\UnknownColorException;
class ColorHelper
{
    ...
    public static function getArrayFromColorString(string $color): array
    {
        ...
        if (preg_match('/\A#[0-9a-fA-F]{3}\z/', $color) === 1) {
            return [(int) hexdec(substr($color, 1, 1) . substr($color, 1, 1)), (int) hexdec(substr($color, 2, 1) . substr($color, 2, 1)), (int) hexdec(substr($color, 3, 1) . substr($color, 3, 1))];
        }
        // #009933 style
        if (preg_match('/\A#[0-9a-fA-F]{6}\z/', $color) === 1) {
            return [(int) hexdec(substr($color, 1, 2)), (int) hexdec(substr($color, 3, 2)), (int) hexdec(substr($color, 5, 2))];
        }
