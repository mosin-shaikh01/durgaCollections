...
use ProductQrBarcode\Vendor\Picqer\Barcode\Exceptions\InvalidCheckDigitException;
use ProductQrBarcode\Vendor\Picqer\Barcode\Exceptions\InvalidFormatException;
...
final class UpcEConverter
{
    public static function normalize(string $code): UpcEData
    {
        ...
        return match (strlen($code)) {
            ...
            default => throw new InvalidLengthException('UPC-E expects 7 or 8 UPC-E digits, or 11 or 12 UPC-A digits.'),
        };
    }
    public static function expand(string $upcE): string
    {
        if (!in_array(strlen($upcE), [7, 8], \true)) {
    ...
    {
        if (!in_array(strlen($upcA), [11, 12], \true)) {
